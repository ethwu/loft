...
; (function (loft) {
    ...
    download.addEventListener('submit', function (e) {
        ...
        if (selected.length < 1) {
            loft.displayMessage('download', 'failure', 'No files selected!');
        } else {
            loft.receive(selected);
        }
    });
    ...
    /**
     * Update the download file listings.
     */
    function updateFileList() {
        loft.list(downloadFailureCallback).then(available => {
    ...
    function downloadFailureCallback(code, msg) {
        loft.displayMessage('download', 'failure', code + ': ' + msg);
    }
